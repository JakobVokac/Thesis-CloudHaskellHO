...
triple (UseProcess n args st) env = do
  nid <- getSelfNode
  env2 <- call' nid $ $(functionDict 'addTwoNums) n (env args)
  _ <- spawnLocal $ do triple st env2
  terminate
  
main :: IO ()
main = do
  Right transport <- createTransport "127.0.0.1" "8091"  (\port' -> ("127.0.0.1" , port')) defaultTCPParameters
  ...
  runProcess node $ do
    ...
    sendChan ps $(mkClosure 'addTwoNums)
    m <- receiveChan pr
    pid1 <- spawnLocal $ do
      triple (SendInt s1 "a" $ SendInt s2 "b" $ RecvInt r3 "c" $ End) ([("a",3),("b",5)])

    pid2 <- spawnLocal $ do
      triple (RecvInt r1 "a" $ RecvInt r2 "b" $ UseProcess m ["a","b"] $ SendInt s3 "c" $ End) ([])
